...
{
    //                             
    bool EINDE_PAD=false; //                                                                              

    while(!EINDE_PAD)
    {
        while(!in_front_of_wall()&&on_ball()) step();
        EINDE_PAD=!found_new_direction();
    }
    if (in_front_of_wall()) turn_around();
    else
    {
        turn_around();
        step();
